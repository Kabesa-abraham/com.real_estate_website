...
import { price } from "../../Assets/Data.js" 
               ...
               rounded-lg' key={index}>
            {
              item.best&& (
                <div className='flex justify-center'>
                  <button className='w-40 h-10 text-white font-medium rounded-full bg-pink-600'>{item.best}</button>
                </div>
              )
            }

            <h3 className="text-lg font-extrabold text-center" >{item.plan}</h3>
            <h1 className="flex items-end gap-1 justify-center font-extrabold text-5xl" >
